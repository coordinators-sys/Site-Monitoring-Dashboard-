...
"""
import os, io, json, sys, re, base64
...
def render(data):
    ...
    # Inline tab icon. The deployed copy also links /favicon-32x32.png, but the standalone
    # file is opened straight from disk with no sibling files, where a data URI is the only
    # thing that resolves. Generated by tools/make_icons.py; absent is not fatal.
    ico=os.path.join("public","favicon-32x32.png")
    if os.path.exists(ico):
        with open(ico,"rb") as fh:
            tpl=tpl.replace("/*FAVICON_INLINE*/",
                            "data:image/png;base64,"+base64.b64encode(fh.read()).decode())
    else:
        print("  NOTE: public/favicon-32x32.png missing — run tools/make_icons.py")
