...
Run 2026-08-21 03:52. Regenerated from live sources.

## Sources & scope
| | Kobo (all partners except IOM) | Zite Manager (IOM) |
|---|---|---|
| In Q2 2026 window | **469** submissions | **723** site records |
| After dedup to site grain | **467** | **719** |
...
**Union: 1186 sites.** 19 duplicate Kobo submissions collapsed
...
2. **Education carries a hard critical floor.** At a site with no learning centre,
   ...
   it is a floor effect, not a gradient. Current split: 600 of 1186 sites are
   critical on that single indicator; the remaining 586 are scored across
   all 11 and distribute normally (G=274 Y=182 R=130).
...
## Validation
- [PASS] sites.json rows (1186) == union (1186)
- [PASS] 467 Kobo + 719 IOM == 1186
- [PASS] every sector cell classified: 14232 == 1186 x 12
...
- [PASS] national severity recomputes (46.8 vs KPI 46.7)
...
- [PASS] source counts agree across provenance/recon/sites.json (467+719 vs recon 467 vs 1186)
- [PASS] age/sex summed over 371 distinct verification rows (371 site rows matched)
...
**Waraabe Gubi** — Baydhaba (IOM)
  - sectors assessed: 11/12 (not assessed: NFI)
  - sector dots: CCCM=G, Protection=R, CP=K, GBV=Y, HLP=G, NFI=NA, Shelter=Y, WASH=R, Health=R, FSL=Y, Nutrition=R, Education=R
  - red/critical sectors: Protection, CP, WASH, Health, Nutrition, Education
  - severity 58 -> band Severe (mean of per-sector red% across the 11 assessed sectors)

## Not done
- **No validated Kobo<->Zite crosswalk.** Master-List matching by code, name and GPS *is*
  performed (748 of 1,186 sites resolved,
  methods: {'code': 281, 'none': 438, 'name': 395, 'gps+name': 78}), and it is what collapses cross-source
