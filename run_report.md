...
Run 2026-08-08 04:12. Regenerated from live sources.

## Sources & scope
| | Kobo (all partners except IOM) | Zite Manager (IOM) |
|---|---|---|
| In Q2 2026 window | **469** submissions | **827** site records |
| After dedup to site grain | **467** | **823** |
...
**Union: 1290 sites.** 19 duplicate Kobo submissions collapsed
...
2. **Education carries a hard critical floor.** At a site with no learning centre,
   ...
   it is a floor effect, not a gradient. Current split: 620 of 1290 sites are
   critical on that single indicator; the remaining 670 are scored across
   all 11 and distribute normally (G=348 Y=190 R=132).
...
## Validation
- [PASS] sites.json rows (1290) == union (1290)
- [PASS] 467 Kobo + 823 IOM == 1290
- [PASS] every sector cell classified: 15480 == 1290 x 12
...
- [PASS] national severity recomputes (45.7 vs KPI 45.7)
...
- [PASS] source counts agree across provenance/recon/sites.json (467+823 vs recon 467 vs 1290)
- [PASS] age/sex summed over 411 distinct verification rows (412 site rows matched)
...
**Misir** — Kismaayo (IOM)
  ...
  - sector dots: CCCM=G, Protection=G, CP=Y, GBV=G, HLP=G, NFI=Y, Shelter=R, WASH=G, Health=G, FSL=Y, Nutrition=G, Education=G
  - red/critical sectors: Shelter
  - severity 23 -> band Low (mean of per-sector red% across the 12 assessed sectors)

## Not done
- **No validated Kobo<->Zite crosswalk.** Master-List matching by code, name and GPS *is*
  performed (826 of 1,290 sites resolved,
  methods: {'code': 281, 'none': 464, 'name': 449, 'gps+name': 102}), and it is what collapses cross-source
