...
Run 2026-08-09 04:19. Regenerated from live sources.

## Sources & scope
| | Kobo (all partners except IOM) | Zite Manager (IOM) |
|---|---|---|
| In Q2 2026 window | **469** submissions | **822** site records |
| After dedup to site grain | **467** | **818** |
...
**Union: 1285 sites.** 19 duplicate Kobo submissions collapsed
...
2. **Education carries a hard critical floor.** At a site with no learning centre,
   ...
   it is a floor effect, not a gradient. Current split: 620 of 1285 sites are
   critical on that single indicator; the remaining 665 are scored across
   all 11 and distribute normally (G=343 Y=190 R=132).
...
## Validation
- [PASS] sites.json rows (1285) == union (1285)
- [PASS] 467 Kobo + 818 IOM == 1285
- [PASS] every sector cell classified: 15420 == 1285 x 12
...
- [PASS] national severity recomputes (45.8 vs KPI 45.8)
...
- [PASS] source counts agree across provenance/recon/sites.json (467+818 vs recon 467 vs 1285)
- [PASS] age/sex summed over 406 distinct verification rows (407 site rows matched)
...
**Wamo 3** — Kismaayo (IOM)
  ...
  - sector dots: CCCM=G, Protection=G, CP=Y, GBV=G, HLP=G, NFI=G, Shelter=Y, WASH=Y, Health=G, FSL=Y, Nutrition=G, Education=G
  - red/critical sectors: none
  - severity 18 -> band Low (mean of per-sector red% across the 12 assessed sectors)

## Not done
- **No validated Kobo<->Zite crosswalk.** Master-List matching by code, name and GPS *is*
  performed (821 of 1,285 sites resolved,
  methods: {'code': 281, 'none': 464, 'name': 445, 'gps+name': 101}), and it is what collapses cross-source
