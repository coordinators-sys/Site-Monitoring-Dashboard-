...
Run 2026-08-26 03:55. Regenerated from live sources.

## Sources & scope
| | Kobo (all partners except IOM) | Zite Manager (IOM) |
|---|---|---|
| In Q2 2026 window | **469** submissions | **301** site records |
| After dedup to site grain | **467** | **299** |
...
**Union: 766 sites.** 19 duplicate Kobo submissions collapsed
...
2. **Education carries a hard critical floor.** At a site with no learning centre,
   ...
   it is a floor effect, not a gradient. Current split: 457 of 766 sites are
   critical on that single indicator; the remaining 309 are scored across
   all 11 and distribute normally (G=125 Y=77 R=107).
...
## Validation
- [PASS] sites.json rows (766) == union (766)
- [PASS] 467 Kobo + 299 IOM == 766
- [PASS] every sector cell classified: 9192 == 766 x 12
...
- [PASS] national severity recomputes (50.0 vs KPI 50.0)
- [PASS] every sector scores its declared indicator count at some site
- [PASS] kpi.catchments (34) == catchAgg rows (34)
- [PASS] Zite label->code translation complete (0 unmapped across 0 labels)
- [PASS] source counts agree across provenance/recon/sites.json (467+299 vs recon 467 vs 766)
- [PASS] age/sex summed over 259 distinct verification rows (259 site rows matched)
...
**Rasmi** — Daynile (IOM)
  - sectors assessed: 11/12 (not assessed: NFI)
  - sector dots: CCCM=Y, Protection=G, CP=Y, GBV=G, HLP=Y, NFI=NA, Shelter=R, WASH=R, Health=Y, FSL=Y, Nutrition=Y, Education=K
  - red/critical sectors: Shelter, WASH, Education
  - severity 49 -> band High (mean of per-sector red% across the 11 assessed sectors)
**Talaal** — Kahda (IOM)
  - sectors assessed: 11/12 (not assessed: NFI)
...
- **No validated Kobo<->Zite crosswalk.** Master-List matching by code, name and GPS *is*
  performed (471 of 766 sites resolved,
  methods: {'code': 281, 'none': 295, 'name': 162, 'gps+name': 32}), and it is what collapses cross-source
