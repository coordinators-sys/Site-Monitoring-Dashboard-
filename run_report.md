...
Run 2026-08-25 03:50. Regenerated from live sources.

## Sources & scope
| | Kobo (all partners except IOM) | Zite Manager (IOM) |
|---|---|---|
| In Q2 2026 window | **469** submissions | **323** site records |
| After dedup to site grain | **467** | **320** |
...
**Union: 787 sites.** 19 duplicate Kobo submissions collapsed
...
2. **Education carries a hard critical floor.** At a site with no learning centre,
   ...
   it is a floor effect, not a gradient. Current split: 464 of 787 sites are
   critical on that single indicator; the remaining 323 are scored across
   all 11 and distribute normally (G=127 Y=87 R=109).
...
## Validation
- [PASS] sites.json rows (787) == union (787)
- [PASS] 467 Kobo + 320 IOM == 787
- [PASS] every sector cell classified: 9444 == 787 x 12
...
- [PASS] national severity recomputes (49.8 vs KPI 49.8)
- [PASS] every sector scores its declared indicator count at some site
- [PASS] kpi.catchments (35) == catchAgg rows (35)
- [PASS] Zite label->code translation complete (0 unmapped across 0 labels)
- [PASS] source counts agree across provenance/recon/sites.json (467+320 vs recon 467 vs 787)
- [PASS] age/sex summed over 276 distinct verification rows (276 site rows matched)
...
**Juba** — Baardheere (IOM)
  ...
  - sector dots: CCCM=R, Protection=K, CP=K, GBV=K, HLP=K, NFI=K, Shelter=K, WASH=Y, Health=R, FSL=Y, Nutrition=G, Education=K
  - red/critical sectors: CCCM, Protection, CP, GBV, HLP, NFI, Shelter, Health, Education
  - severity 79 -> band Severe (mean of per-sector red% across the 12 assessed sectors)
**Daacad** — Kahda (IOM)
  - sectors assessed: 11/12 (not assessed: NFI)
  - sector dots: CCCM=Y, Protection=G, CP=Y, GBV=G, HLP=Y, NFI=NA, Shelter=Y, WASH=R, Health=Y, FSL=R, Nutrition=G, Education=K
  - red/critical sectors: WASH, FSL, Education
  - severity 45 -> band High (mean of per-sector red% across the 11 assessed sectors)

## Not done
- **No validated Kobo<->Zite crosswalk.** Master-List matching by code, name and GPS *is*
  performed (488 of 787 sites resolved,
  methods: {'code': 281, 'none': 299, 'name': 177, 'gps+name': 35}), and it is what collapses cross-source
